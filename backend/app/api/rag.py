...
from app.services.chat_service import execute_chat, persist_messages
...
async def chat_with_documents(
    ...
    # 2. Execute chat
    chat_result = await execute_chat(
        workspace_id=workspace_id,
        question=request.message,
        context=retrieval_result.context,
        sources=retrieval_result.sources,
        image_refs=retrieval_result.image_refs,
        image_parts=retrieval_result.image_parts,
    ...
    # 3. Extract related entities from KG (inline, simple)
    related_entities = []
    if chat_result.kg_summary:
        try:
            kg = await _get_kg_service(workspace_id)
            entities = await kg.get_entities(limit=200)
            entity_names = {e["name"].lower(): e["name"] for e in entities}
            answer_lower = chat_result.answer.lower()
            for lower_name, original_name in entity_names.items():
                if len(lower_name) >= 2 and lower_name in answer_lower:
    ...
    await persist_messages(
        ...
        image_refs=chat_result.image_refs,
    ...
    return ChatResponse(
        ...
        kg_summary=chat_result.kg_summary,
        image_refs=chat_result.image_refs,
        thinking=chat_result.thinking,
    )
